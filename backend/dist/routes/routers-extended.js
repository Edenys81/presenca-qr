...
export const analysisRouter = router({
    ...
    generateFrequencyAnalysis: adminProcedure.mutation(async ({ ctx }) => {
        ...
        await db.createAnalysis({
            tipo: "frequencia",
            conteudo: typeof analysis === "string"
                ? analysis
    ...
    generateImprovementSuggestions: adminProcedure.mutation(async ({ ctx }) => {
        ...
        await db.createAnalysis({
            tipo: "sugestoes",
            conteudo: typeof suggestions === "string"
                ? suggestions
...
export const certificateRouter = router({
    ...
    generateCertificate: protectedProcedure
        ...
        const result = await certificates.generateCertificatePDF(input.studentId, input.eventId, input.attendanceId);
        // notificar aluno
        const event = await db.getEventById(input.eventId);
        if (event) {
            await services.notifyStudentCertificateAvailable(input.studentId, event.nome);
        }
        return result;
    }),
